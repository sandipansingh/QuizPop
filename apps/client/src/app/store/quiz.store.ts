...
interface QuizState {
  ...
  resetQuiz: () => void;
  loadQuestions: (params?: GetQuestionsParams) => Promise<Question[]>;
  answerCurrentQuestion: (payload: {
    selectedAnswer: string;
    timeTakenMs: number;
  }) => void;
  nextQuestion: () => number;
  submitQuiz: (options?: { roomId?: string }) => Promise<QuizSubmitResult>;
  setSubmitResult: (submitResult: QuizSubmitResult | null) => void;
}
...
export const useQuizStore = create<QuizState>((set, get) => ({
  ...
  loadQuestions: async (params = {}) => {
    ...
    try {
      ...
      set({
        questions: [],
        isLoadingQuestions: false,
        errorMessage: err.message ?? 'Failed to fetch quiz questions.',
      });
      throw error;
    }
  },
  ...
  submitQuiz: async ({ roomId } = {}) => {
    ...
    try {
      ...
      set({
        isSubmittingQuiz: false,
        errorMessage: err.message ?? 'Quiz submission failed.',
      });
      throw error;
    }
  },
